...
class AnalysisGraphService:
    ...
    def ask(
        ...
        # 0. HARD ROUTE ANALÍTICA / DETERMINÍSTICA.
        # Perguntas de contagem, ranking, agrupamento, comparação e KPI não podem cair em RAG/LLM.
        # O LLM pode formatar texto em outros fluxos, mas não deve recalcular nem sintetizar números.
        try:
        ...
        # 0.1. Se for pergunta analítica e a engine estruturada não respondeu,
        # tenta tabular uma única vez e retorna direto. Não busca chunks e não chama OpenAI.
        if is_analytics:
        ...
        # 1. Recupera evidências do case atual apenas para perguntas semânticas/descritivas.
        # Analytics já foi resolvido ou bloqueado acima.
        case_evidences: list[dict[str, Any]] = []
        if documents:
            case_evidences = self.retrieval_service.search(case_id, question, top_k=6)
            case_evidences = self._tag_evidences(case_evidences, default_source="case_upload")

        # 2. Executa consulta tabular híbrida somente como apoio para perguntas não analíticas.
        tabular_result = self.tabular_service.answer_question(
            case_id=case_id,
            question=question,
            documents=documents,
