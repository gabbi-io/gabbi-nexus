...
class RetrievalService:
    def __init__(self):
        ...
        self.case_upload_boost = float(os.getenv("GABBI_CASE_UPLOAD_BOOST", "0.35"))
        self.knowledge_boost = float(os.getenv("GABBI_KNOWLEDGE_BASE_BOOST", "0.05"))
    ...
    def build_case_index(self, case_id: str, documents: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        vectorizer = TfidfVectorizer(stop_words=None, ngram_range=(1, 2), token_pattern=r"(?u)\b\w[\w:/\-\.]+\b")
        matrix = vectorizer.fit_transform(texts)
        self.case_vectorizers[case_id] = vectorizer
        self.case_matrices[case_id] = matrix

        publish_info = {"published": False, "backend": self.vector_backend, "chunks": len(chunks), "matrix_shape": list(matrix.shape)}
        if self.vector_backend == "pgvector" and self._table is not None and self.openai_api_key:
            try:
                self._publish_pgvector(case_id, chunks)
                publish_info.update({"published": True, "backend": "pgvector"})
            except Exception as exc:
                publish_info.update({"published": False, "backend": "pgvector", "error": str(exc)})
        return publish_info

    def search(self, case_id: str, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        # Local é fonte obrigatória porque contém o índice recém-reconstruído do case atual.
        local_found = self._search_local(case_id, query, max(top_k, 5))
        if local_found:
            return local_found[:top_k]
    ...
    def _search_local(self, case_id: str, query: str, top_k: int) -> list[dict[str, Any]]:
        ...
        # Se a pergunta cita arquivo/anexo, garante retorno de uploads mesmo com score lexical baixo.
        q = str(query or "").lower()
        wants_file = any(t in q for t in ["arquivo", "anexo", "documento", "planilha", "upload", "pdf", "txt", "csv", "xlsx"])
        if wants_file:
    ...
    def _source_boost(self, chunk: ChunkItem) -> float:
        ...
        if any(filename.endswith(ext) for ext in [".xlsx", ".xls", ".csv", ".pdf", ".docx", ".txt", ".md"]):
    ...
    def _rerank_results(self, found: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        for item in found:
            ...
            pseudo = ChunkItem(str(cloned.get("chunk_id", "")), str(cloned.get("filename", "")), str(cloned.get("excerpt", "")), metadata=metadata)
    ...
    def _search_pgvector(self, case_id: str, query: str, top_k: int) -> list[dict[str, Any]]:
        ...
        ).where(self._table.c.case_id == case_id).order_by(distance).limit(top_k)
        with Session(self._db_engine) as session:
