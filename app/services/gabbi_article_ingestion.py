...
class GabbiArticleIngestionService:
    ...
    def build_documents_from_articles(self, limit: int = 1000, updated_after: datetime | None = None) -> list[dict]:
        articles = self.article_repository.list_published_articles(limit=limit, updated_after=updated_after)
        documents: list[dict] = []
        for article in articles:
            ...
            documents.append({
                ...
                "structured_row": parse_article_to_structured_row(
                    ...
                    metadata=metadata,
                    defaults={"article_id": article.id, "article_ref_id": article.ref_id, "topic_id": article.topic_id, "topic_name": article.topic_name},
                ),
            })
        return documents
