...
class GabbiPostgresIngestionService:
        ...
        *,
        ...
        for article in articles:
            ...
            structured_row = parse_article_to_structured_row(
                ...
                article_document=getattr(article, "document", None),
                metadata=metadata,
                defaults={
                    "article_id": article.id,
                    "article_ref_id": article.ref_id,
                    "topic_id": article.topic_id,
                    "topic_name": article.topic_name,
                    "topic_description": article.topic_description,
                    "article_updated_on": article.updated_on.isoformat() if article.updated_on else None,
                },
            )
    ...
    def _build_article_text(self, article) -> str:
        ...
        if article.document:
            ...
            parts.append(article.document if isinstance(article.document, str) else str(article.document))
        return "\n\n".join(parts)
